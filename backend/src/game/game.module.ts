...
@Module({
  ...
  providers: [GameGateway, AuthService, GameService, UserService, GamesUtileService, GamePortal, GameCalculation]
})
...
//   createGame(multiGameId: string, gameMode: number, p1SocketId: string, p2SocketId:string, user1: User, user2:User) : GameId {
//     let game: GameId;

//     game.user1 = user1;
//     game.user2 = user2;
//     game.gameMode = gameMode;
//     game.multiGameId = multiGameId;
//     game.gameStatus = 0; //0: game stopped, 1: game in progress, 2: player disconnected
//     game.once = 0; //is used to make sure only one client triggers the start og the game
//     game.p1SocketId = p1SocketId;
//     game.p2SocketId = p2SocketId;
//     game.portalOn = false;
//     game.count = 3;
//     game.goalIsProcessed = true; //is used to make sure the previous goal has been processed before resuming the game, it could cause issues with the ball position without it
//     game.countdownDone = false;
//     game.ball = {
//       posX: 0,
//       posY: 0,
...
//         runningGames.push(this.createGame(res, gameMode, waitRoomLength[0].socket, socket.id, user, user2))
//         waitRoomNormal = [];
//       }
//       else
//       {
//         //runningGames.push(new Game(res, gameMode, waitRoomPortal[0].socket, socket.id, user, waitRoomPortal[0].user));
//         runningGames.push(this.createGame(res, gameMode, waitRoomLength[0].socket, socket.id, user, waitRoomPortal[0].user))
//         waitRoomPortal = [];
//       }
//     }
//     return res;
