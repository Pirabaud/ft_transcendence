...
{
  let count = this.gameUtile.countValues(gameId, this.knownClients);
  socket.join(gameId);
  if (count === 0)
...
createGame(multiGameId: string, gameMode: number, firstPlayer: UserWaiting, secondPlayer: UserWaiting) : GameId {
    let game: GameId = {
  ...
  checkGameAvailability(server: Server, gameMode: number, userWaiting: UserWaiting) {
    ...
    waitRoomLength = gameMode === 0 ? this.waitRoomNormal.length : this.waitRoomPortal.length;
    if (waitRoomLength === 0) {
      ...
      const gameReadyCheck = setInterval(() => {
        let i: number = this.gameUtile.getGameIndex(userWaiting.gameId, "gameId", this.runningGames);
        if (i !== -1) {
  ...
  startGame(server: Server, socket: any, gameId: string) {
    let obj: any = this.knownClients.get(socket.id);
    let index: number = this.gameUtile.getGameIndex(gameId, "gameId", this.runningGames);
    ...
    if (game.once === 0) {
      ...
      const ballLoop = setInterval(() => {
        if (game.gameStatus === 1 && game.countdownDone) {
          game.gameStatus = this.gameCalculation.ballMovement(server, gameId, this.runningGames);
  ...
  * the data is sent back to the clients*/
  updatePaddlePos(socket: Socket, gameId: string, data: any, clientHeight: number) {
    ...
    if (clientId.clientId === "0") {
      ...
      this.runningGames[index].paddle2.posY = this.runningGames[index].paddle2.posY * (600 / clientHeight);
  ...
  assignPaddle(socket: Socket) {
    let obj = this.knownClients.get(socket.id);
